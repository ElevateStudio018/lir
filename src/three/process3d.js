...
// Procedural surface texture (concrete / timber variation) — no network, no textures to download.
function noiseTexture(size, base, spread, grain = false) {
  const c = document.createElement('canvas');
  c.width = c.height = size;
  const g = c.getContext('2d');
  const img = g.createImageData(size, size);
  const r = rng(size + spread);
  for (let y = 0; y < size; y++) {
    for (let x = 0; x < size; x++) {
      const i = (y * size + x) * 4;
      let n = (r() - 0.5) * spread;
      if (grain) n += Math.sin(x * 0.18 + Math.sin(y * 0.03) * 6) * spread * 0.6;
...
export function createProcessScene(canvas, { mobile = false, reduced = false } = {}) {
  ...
  // ---------- Materials ----------
  const concreteTex = noiseTexture(256, 188, 34);
  concreteTex.repeat.set(2, 2);
  const woodTex = noiseTexture(256, 150, 40, true);
  const groundTex = noiseTexture(256, 215, 46);
  ...
  const M = {
    ground: std({ color: 0x151c1f, roughness: 1, map: groundTex }),
    soil: std({ color: 0x3b342d, roughness: 1, side: THREE.BackSide }),
    lid: std({ color: 0x151c1f, roughness: 1, transparent: true }),
    concrete: std({ color: 0xbab6ab, roughness: 0.86, map: concreteTex }),
    slab: std({ color: 0xa9a59b, roughness: 0.9, map: concreteTex }),
    steel: std({ color: 0x3e464b, roughness: 0.42, metalness: 0.75 }),
    panel: std({ color: 0xd9d5ca, roughness: 0.74, map: concreteTex }),
    wood: std({ color: 0xa27a52, roughness: 0.62, map: woodTex }),
    glass: std({ color: 0x31434d, roughness: 0.1, metalness: 0.55, envMapIntensity: 1.1 }),
    glassLit: std({ color: 0x33434b, roughness: 0.12, metalness: 0.5, envMapIntensity: 1.0, emissive: 0xffb36b, emissiveIntensity: 0 }),
    crane: std({ color: 0xc39c45, roughness: 0.5, metalness: 0.35 }),
    craneDark: std({ color: 0x2a2f33, roughness: 0.6, metalness: 0.4 }),
    plaza: std({ color: 0xb9b3a5, roughness: 0.95, transparent: true, opacity: 0 }),
    asphalt: std({ color: 0x2a2e30, roughness: 0.95, transparent: true, opacity: 0 }),
  ...
  const ground = new THREE.Mesh(new THREE.ShapeGeometry(gShape), M.ground);
  ground.rotation.x = -Math.PI / 2;
  ground.receiveShadow = true;
  root.add(ground);

  const pit = new THREE.Mesh(new THREE.BoxGeometry(PIT_W, PIT_H, PIT_D), M.soil);
  pit.position.y = -PIT_H / 2 - 0.001;
  pit.receiveShadow = true;
  root.add(pit);

  const lid = new THREE.Mesh(new THREE.PlaneGeometry(PIT_W, PIT_D), M.lid);
  ...
  const footings = new THREE.InstancedMesh(new THREE.BoxGeometry(1.8, 2.6, 1.8), M.concrete, NC);
  footings.castShadow = footings.receiveShadow = true;
  root.add(footings);

  const slab0 = new THREE.Mesh(new THREE.BoxGeometry(W + 1, 0.5, D + 1), M.slab);
  ...
  // ---------- Structure ----------
  const COLS = NC * F;
  const columns = new THREE.InstancedMesh(new THREE.BoxGeometry(0.42, 1, 0.42), M.concrete, COLS);
  ...
  const slabs = [];
  for (let f = 1; f <= F; f++) {
    const s = new THREE.Mesh(new THREE.BoxGeometry(W + 0.8, 0.32, D + 0.8), M.slab);
  ...
  const core = new THREE.Mesh(new THREE.BoxGeometry(3.6, coreH, 4.2), M.concrete);
  ...
  const spandrels = new THREE.InstancedMesh(new THREE.BoxGeometry(1, 1, 0.26), M.panel, spandrelBays.length);
  ...
  const finMesh = new THREE.InstancedMesh(new THREE.BoxGeometry(0.14, 1, 0.56), M.wood, fins.length);
  ...
  const parapetGeo = mergeGeometries([
    new THREE.BoxGeometry(W + 1.4, 0.9, 0.3).translate(0, 0, D / 2 + 0.55),
    new THREE.BoxGeometry(W + 1.4, 0.9, 0.3).translate(0, 0, -D / 2 - 0.55),
    new THREE.BoxGeometry(0.3, 0.9, D + 1.4).translate(W / 2 + 0.55, 0, 0),
    new THREE.BoxGeometry(0.3, 0.9, D + 1.4).translate(-W / 2 - 0.55, 0, 0),
  ]);
  ...
  const plant = new THREE.Mesh(new THREE.BoxGeometry(5, 1.8, 3.2), M.panel);
  ...
  // ---------- Landscape ----------
  const plaza = new THREE.Mesh(new THREE.PlaneGeometry(44, 30), M.plaza);
  plaza.rotation.x = -Math.PI / 2;
  plaza.position.set(0, 0.03, 2);
  plaza.receiveShadow = true;
  root.add(plaza);

  const road = new THREE.Mesh(new THREE.PlaneGeometry(260, 8), M.asphalt);
  ...
  const paperMat = new THREE.ShaderMaterial({
    ...
    uniforms: {
      ...
      uGrid: { value: new THREE.Color(0x0b1113) },
      uGridAmt: { value: 0 },
  ...
  const groundNight = new THREE.Color(0x182023);
  const groundDay = new THREE.Color(0x6f735a);
  ...
  function apply(t) {
    ...
    for (let f = 0; f < F; f++) {
      ...
      for (let c = 0; c < NC; c++) {
        ...
        const h = FH - 0.32;
        const sy = h * cq;
        setInst(columns, f * NC + c, colPos[c][0], f * FH + 0.3 + sy / 2, colPos[c][1], 1, sy, 1);
      }
    }
    ...
    fins.forEach((fn, i) => {
      ...
      const h = (F * FH + 0.1) * k;
      setInst(finMesh, i, fn.x, 0.3 + h / 2, fn.z, 1, h, 1, fn.ry);
    });
    ...
    M.concrete.color.set(0xbab6ab).lerp(tmpC.set(0xcfcac0), pFacade);
    ...
    const toBlue = ease.inOut(sub(pBp, 0.5, 0.68));
    const toInk = ease.inOut(sub(pBp, 0.82, 1));
    paperMat.uniforms.uOpacity.value = toPaper;
    paper.visible = toPaper > 0.001;
    paperMat.uniforms.uColor.value.set(0xf4f3ef).lerp(tmpC.set(0x10283b), toBlue).lerp(tmpC.set(0x161b1d), toInk);
    paperMat.uniforms.uGrid.value.set(0x0b1113).lerp(tmpC.set(0x9cc7e6), toBlue);
    paperMat.uniforms.uGridAmt.value = sub(pBp, 0.4, 0.6) * (1 - toInk);
    ...
    edgeMat.uniforms.uColor.value.set(0x1d2427).lerp(tmpC.set(0xdcebf6), toBlue);
  ...
  function buildEdges() {
    ...
    const inst = [columns, spandrels, glassDark, glassLit, finMesh, trunks, solar, lampPoles];
