/**
 * MATERIAL TRANSITION — "Detaljer skapar helheten."
 *
 * One full-screen fragment shader renders a procedural precast-concrete facade at
 * any scale: from a 6 cm macro view of pores and aggregate, via panel joints and
 * window reveals, out to the whole building at dusk. Noise octaves are filtered by
 * pixel footprint so the zoom never shimmers. Scroll drives the zoom directly.
 */
const VERT = `
attribute vec2 aPos;
...
export function createMaterialScene(canvas, { mobile = false } = {}) {
  ...
  const scale = mobile ? Math.min(window.devicePixelRatio || 1, 1.5) * 0.6 : Math.min(window.devicePixelRatio || 1, 1);
  let zTarget = 0;
  let zNow = 0;
  let running = false;
  let raf = 0;
  ...
  function frame(now) {
    ...
    const prev = zNow;
    zNow += (zTarget - zNow) * 0.12;
    if (Math.abs(zTarget - zNow) < 0.0001) zNow = zTarget;
    if (!dirty && prev === zNow) return; // static — skip redundant draws
    dirty = false;
    gl.uniform1f(uZoom, zNow);
